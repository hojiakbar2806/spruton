...
import { closeAddBox, openAddBox } from "../../context/slice/addBoxSlice";
import { useUpdateBoxQuestsMutation } from "../../context/service/box.service";
import { Toast } from "primereact/toast";
import { useGetQuestByIdQuery } from "../../context/service/quest.service";

export const ShareBox = () => {
  const id = useSelector((state) => state.box.sharedQuestId);
  const [update] = useUpdateBoxQuestsMutation();
  const { data: currentQuest = null } = useGetQuestByIdQuery(id, {
    skip: !id,
  });
  ...
  const show = useSelector((state) => !!state.box.sharedQuestId);
  ...
  useEffect(() => {
    if (currentQuest) {
      const { description, channel_link, status } = currentQuest.innerData;
